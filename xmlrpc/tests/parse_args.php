...
class argParser {
	public static function getArgs() {
		...
		$args = array('DEBUG' => 0, 'LOCALSERVER' => 'localhost', 'HTTPSSERVER' => 'gggeek.ssl.altervista.org', 'HTTPSURI' => '/sw/xmlrpc/demo/server/server.php', 'HTTPSIGNOREPEER' => false, 'HTTPSVERIFYHOST' => 2, 'SSLVERSION' => 0, 'PROXYSERVER' => null, 'NOPROXY' => false, 'LOCALPATH' => __DIR__,);
		...
		if (!isset($URI)) {
			...
			} elseif (isset(basename(__FILE__)) && isset($_SERVER['REQUEST_METHOD'])) {
				$URI = str_replace('/tests/testsuite.php', '/demo/server/server.php', basename(__FILE__));
				$URI = str_replace('/testsuite.php', '/server.php', $URI);
				$URI = str_replace('/tests/benchmark.php', '/demo/server/server.php', $URI);
				$URI = str_replace('/benchmark.php', '/server.php', $URI);
			} else {
