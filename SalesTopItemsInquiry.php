...
echo '<fieldset>
		<legend>', _('Display'), '</legend>'; //new sub table to set parameters for order of display
if (!isset($_POST['OrderBy'])) { //default to order by net sales
	$_POST['OrderBy'] = 'NetSales';
}
echo '<field>
		<label for="NetSales">', _('Order By Net Sales'), ':</label>
...
echo '<field>
		...
		<input type="text" class="number" name="NoToDisplay" size="4" required="required" maxlength="4" value="', $_POST['NoToDisplay'], '"  />
	</field>';
