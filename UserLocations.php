...
echo '<p class="page_title_text">
		<img src="', $RootPath, '/css/', $_SESSION['Theme'], '/images/money_add.png" title="', _('User Authorised Locations'), '" alt="" />', ' ', $Title, '
	</p>';
...
if (isset($_POST['process']) or isset($SelectedUser)) {
	...
	echo '<div class="centre">
			<a href="', htmlspecialchars(basename(__FILE__), ENT_QUOTES, 'UTF-8'), '">', _('Authorised inventory locations for'), ' ', $SelectedUserName, '</a>
		</div>';
	...
	while ($MyRow = DB_fetch_array($Result)) {
		...
		if ($MyRow['canupd'] == 1) {
			$ToggleText = '<td><a href="' . htmlspecialchars(basename(__FILE__), ENT_QUOTES, 'UTF-8') . '?SelectedLocation=' . urlencode($MyRow['loccode']) . '&amp;ToggleUpdate=0&amp;SelectedUser=' . urlencode($SelectedUser) . '" onclick="return confirm(\'' . _('Are you sure you wish to remove Update for this location?') . '\');">' . _('Remove Update') . '</a></td>';
			$Update = _('Yes');
		} else {
			$ToggleText = '<td><a href="' . htmlspecialchars(basename(__FILE__), ENT_QUOTES, 'UTF-8') . '?SelectedLocation=' . urlencode($MyRow['loccode']) . '&amp;ToggleUpdate=1&amp;SelectedUser=' . urlencode($SelectedUser) . '" onclick="return confirm(\'' . _('Are you sure you wish to add Update for this location?') . '\');">' . _('Add Update') . '</a></td>';
			$Update = _('No');
		}
		...
		echo '<tr class="striped_row">
				...
				<td><a href="', htmlspecialchars(basename(__FILE__), ENT_QUOTES, 'UTF-8'), '?SelectedLocation=', urlencode($MyRow['loccode']), '&amp;delete=yes&amp;SelectedUser=', urlencode($SelectedUser), '" onclick="return confirm(\'', _('Are you sure you wish to un-authorise this location?'), '\');">', _('Un-authorise'), '</a></td>
			</tr>';
	}
	//END WHILE LIST LOOP
	echo '</tbody>
	...
	if (!isset($_GET['delete'])) {
		...
		echo '<fieldset>
				<legend>', _('Stock Locations'), '</legend>'; //Main table
