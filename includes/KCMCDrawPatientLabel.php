...
echo ' <canvas id="PatientLabel" width="282" height="178" style="border:1px solid #000000;background-color:#ffffff;"></canvas>';
...
ob_start();
imagepng(imagepng($BarcodeImage->draw(false)));
$Image_String = ob_get_contents();
ob_end_clean();
file_put_contents('companies/KwaMoja/barcodes_dir/' . $PID . '.png', $Image_String);
...
ctx.font = "14px Arial";
ctx.fillText("', _('Patient ID'), ' - ', $PID, '", 10, 20);
...
BarCode.src = "companies/' . $_SESSION['DatabaseName'] . '/barcodes_dir/' . $PID . '.png";

BarCode.onload = function() {
    ctx.drawImage(BarCode, 10, 130, 250, 50);
}
