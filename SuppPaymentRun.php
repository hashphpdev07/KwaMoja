...
if ((isset($_POST['PrintPDF']) or isset($_POST['PrintPDFAndProcess'])) and isset($_POST['FromCriteria']) and mb_strlen($_POST['FromCriteria']) >= 1 and isset($_POST['ToCriteria']) and mb_strlen($_POST['ToCriteria']) >= 1 and is_numeric(filter_number_format($_POST['ExRate']))) {
	...
	echo '<field>
			...
			<input type="text" required="required" maxlength="6" size="7" name="FromCriteria" value="', $DefaultFromCriteria, '" />
		  </field>';

	echo '<field>
			<label for="ToCriteria">', _('To Supplier Code'), ':</label>
			<input type="text" required="required" maxlength="6" size="7" name="ToCriteria" value="', $DefaultToCriteria, '" />
		 </field>';
	...
	echo '<field>
			...
			<input type="text" class="number" name="ExRate" required="required" maxlength="11" size="12" value="', locale_number_format($DefaultExRate, 'Variable'), '" /></label>
		  </field>';
	...
	echo '<field>
			...
			<input type="text" class="date" name="AmountsDueBy" required="required" maxlength="11" size="12" value="', $DefaultDate, '" />
		  </field>';
	...
	echo '<field>
			...
			<select required="required" name="BankAccount">';
	if (DB_num_rows($AccountsResults) == 0) {
		echo '</select>
			</field>
		</fieldset>';

		echo '<p>', _('Bank Accounts have not yet been defined. You must first'), ' <a href="', $RootPath, '/BankAccounts.php">', _('define the bank accounts'), '</a> ', _('and general ledger accounts to be affected'), '.
			</p>';
		include ('includes/footer.php');
		exit;
	} else {
	...
	echo '<field>
			...
			<select required="required" name="PaytType">';
