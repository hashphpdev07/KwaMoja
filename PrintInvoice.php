...
while ($FromTransNo <= filter_number_format($_POST['ToTransNo'])) {
	...
	if (DB_num_rows($Result) == 1) {
		...
		/* Get the correct Invoice address */
		...
		echo '<table class="Main" title="', _('Main table for Invoice'), '">
				<thead>
					<tr>
					...
					<tr>
						<td colspan="2" class="TransType">
							<div class="centre">
							<img class="barcode" alt="Invoice - ', $FromTransNo, '" src="includes/barcode.php?text=', $FromTransNo, '" />
							', _('Tax Invoice'), '</div>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2" class="InvoiceDetails">
							<table>
								<tr>
									<td>', _('Invoice No'), ':  ', $FromTransNo, '</td>
									<td>', _('Order Date'), ':  ', ConvertSQLDate($MyRow['orddate']), '</td>
									<td>', _('Invoice Date'), ':  ', ConvertSQLDate($MyRow['trandate']), '</td>
