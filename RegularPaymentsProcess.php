...
if (DB_num_rows($Result) > 0 and !isset($_GET['Edit'])) {
	...
	while ($MyRow = DB_fetch_array($Result)) {
		...
		$ExRateResult = DB_query("SELECT rate FROM currencies WHERE currabrev='" . $MyRow['currabrev'] . "'");
		$ExRateRow = DB_fetch_row($ExRateResult);
		$tableExRate = $ExRateRow[0]; //this is the rate of exchange between the functional currency and the payment currency
